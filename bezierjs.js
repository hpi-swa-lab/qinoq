...
const utils = {
  ...
  pairiteration: function (c1, c2, curveIntersectionThreshold) {
    ...
    const c2b = c2.bbox();
    const r = 100000;
    const threshold = curveIntersectionThreshold || 0.5;
  ...
  getccenter: function (p1, p2, p3) {
    ...
    let m = atan2(p2.y - arc.y, p2.x - arc.x);
...
class Bezier {
  ...
  static getABC (order = 2, S, B, E, t = 0.5) {
    ...
    const s = utils.abcratio(t, order);
    const A = {
      x: B.x + (B.x - C.x) / s,
      y: B.y + (B.y - C.y) / s
    };
  ...
  __normal3 (t) {
    ...
    const q1 = sqrt$1(r1.x * r1.x + r1.y * r1.y + r1.z * r1.z);
    const q2 = sqrt$1(r2.x * r2.x + r2.y * r2.y + r2.z * r2.z);
    r1.x /= q1;
    r1.y /= q1;
    r1.z /= q1;
    r2.x /= q2;
  ...
  outline (d1, d2, d3, d4) {
    ...
    const ls = utils.makeline(bs, fs);
    const le = utils.makeline(fe, be);
  ...
  _error (pc, np1, s, e) {
    ...
    const ref = utils.dist(pc, np1);
    const d1 = utils.dist(pc, c1);
    const d2 = utils.dist(pc, c2);
